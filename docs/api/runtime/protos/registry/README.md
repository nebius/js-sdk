[**@nebius/nodejs-sdk**](../../../README.md)

---

[@nebius/nodejs-sdk](../../../README.md) / runtime/protos/registry

# runtime/protos/registry

## Classes

- [Registry](classes/Registry.md)

## Interfaces

- [EnumClassInterface](interfaces/EnumClassInterface.md)
- [EnumInstanceInterface](interfaces/EnumInstanceInterface.md)
- [ExtensionDescriptor](interfaces/ExtensionDescriptor.md)
- [MessageClassInterface](interfaces/MessageClassInterface.md)
- [MessageInstanceInterface](interfaces/MessageInstanceInterface.md)
