...
import { printExtensions } from './printExtensions.js';
import { printMessage } from './printMessage/index.js';
import { printService, type TypeIndex } from './printService.js';
import { withTypeNameContext } from './typeNames.js';

function toJsModuleSpecifier(specifier: string): string {
  if (!specifier.startsWith('.')) return specifier;
  if (specifier.endsWith('.js')) return specifier;
  return `${specifier}.js`;
}

function toGeneratedIndexModuleSpecifier(specifier: string): string {
  return toJsModuleSpecifier(specifier.endsWith('/index') ? specifier : `${specifier}/index`);
}
...
export function generateIndexContent(
  ...
  entries: {
    ...
    extensions?: import('../descriptors.js').Field[];
  }[],
  typeIndex: TypeIndex,
  ...
  _extensionMap?: Map<string, import('../descriptors.js').Field[]>,
): string {
  const lines: string[] = [];
  ...
  const registryRelPath = (() => {
    ...
    return toJsModuleSpecifier(full.startsWith('.') ? full : `./${full}`);
  })();
  ...
  if (outBase) {
    ...
    runtimeImport = toJsModuleSpecifier(runtimeImport);
    ...
    runtimeImport = toJsModuleSpecifier(`${up}/runtime/protos/index`);
  }
  ...
  typeNameOverrides.set('.google.protobuf.Empty', 'WktEmpty');
  ...
  for (const { services, pkg } of entries) {
    ...
    for (const s of services) {
      for (const m of s.methods) {
        ...
        if (inInfo && inInfo.dir !== (dir || '.') && inKey !== '.google.protobuf.Empty') {
          addImport(inInfo.dir, inInfo.tsNameOriginal ?? inInfo.tsName, inKey);
        }
        if (outInfo && outInfo.dir !== (dir || '.') && outKey !== '.google.protobuf.Empty') {
  ...
  if (hasServices) {
    ...
    lines.push(`import type { SDKInterface } from "${toJsModuleSpecifier(`${up}/sdk`)}";`);
    lines.push(
  ...
  if (importMap.size) {
    ...
    for (const tDir of sortedDirs) {
      ...
      lines.push(`import { ${names.join(', ')} } from "${toGeneratedIndexModuleSpecifier(rel)}";`);
    }
  }
