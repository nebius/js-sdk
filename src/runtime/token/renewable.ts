...
export class RenewableBearer extends Bearer {
  ...
  receiver(): Receiver { return new RenewableReceiver(this, this.maxRetries); }
  ...
  private needRenew(now = Date.now()): boolean {
    if (!this.cacheToken) { dbg('needRenew -> true (no token)'); return true; }
    ...
    dbg('needRenew', { remaining, safetyMinRemainingMs: this.safetyMinRemainingMs, isExpired: this.cacheToken.isExpired(), result: res });
    return res;
  }
    ...
    if (mustRenew) {
      if (renewSynchronous) {
        ...
        this.nextSyncOptions = {
          timeoutMs: typeof options?.renewRequestTimeoutMs === 'number'
