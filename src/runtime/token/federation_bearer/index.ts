...
class FederationReceiver extends Receiver {
  ...
  protected async _fetch(timeoutMs?: number, _options?: AuthorizationOptions | undefined): Promise<Token> {
    ...
    dbg('receiver._fetch: authorize result', { expires_in: res?.expires_in, has_access_token: !!res?.access_token });
  ...
  canRetry(_err: unknown, _options?: AuthorizationOptions | undefined): boolean { dbg('receiver.canRetry -> false'); return false; }
}
...
export class FederationBearer extends Bearer {
  ...
  receiver(): Receiver {
    dbg('bearer.receiver');
    return new FederationReceiver(this.clientId, this.federationEndpoint, this.federationId, this.writer, this.noBrowserOpen, this.ca);
  }
}
